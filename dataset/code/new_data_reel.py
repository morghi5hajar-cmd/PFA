...
DB_PATH = "pharmaciee.db"

BOITES_PAR_PERSONNE = 12.26
DEPENSE_PAR_PERSONNE = 500
ANNEE = 2024

# (id_region, id_grossiste, nb_pharmacies, nb_patients)
VILLES_CONFIG = {
    "Rabat":       (1, 1,  2,  20),
    "Salé":        (1, 1,  2,  40),
...
# Répartition RÉELLE du marché marocain
# Source : La Quotidienne + Cour des comptes Maroc 2024
# T1 : 98.8% du volume des ventes
# T2 : 0.9% | T3 : 0.2% | T4 : 0.1%
NB_MEDS = {"T1": 988, "T2": 9, "T3": 2, "T4": 1}

NOMS_T1 = [
    "Paracétamol", "Amoxicilline", "Ibuprofène", "Doliprane", "Metformine",
    "Oméprazole", "Cétirizine", "Vitamine C", "Loratadine", "Azithromycine",
    "Fluconazole", "Metronidazole", "Antalgique", "Aspirine", "Spasfon",
    "Smecta", "Toplexil", "Voltarène", "Zyrtec", "Augmentin"
]
NOMS_T2 = [
    "Ventoline", "Insuline", "Atorvastatine", "Losartan", "Amlodipine",
    "Metoprolol", "Furosémide", "Lisinopril", "Simvastatine", "Ramipril"
]
NOMS_T3 = ["Rituximab", "Trastuzumab"]
NOMS_T4 = ["Sutent"]
...
def gen_medicaments():
    ...
    config = {
        ...
        "T2": (NOMS_T2, 166,  588),
        "T3": (NOMS_T3, 589, 1756),
        "T4": (NOMS_T4, 1757, 4000),
    }
    for cat, nb in NB_MEDS.items():
        noms_base, pmin, pmax = config[cat]
        for i in range(nb):
            base = random.choice(noms_base)
            compteurs[base] = compteurs.get(base, 0) + 1
            nom  = f"{base} {random.choice(DOSAGES)} {random.choice(FORMES)} ({compteurs[base]})"
...
try:
    ...
    cursor.execute("""CREATE TABLE medicaments (
        id_medicament INTEGER PRIMARY KEY AUTOINCREMENT,
        nom           TEXT NOT NULL,
        categorie     TEXT,
        pfht          REAL)""")

    cursor.execute("""CREATE TABLE pharmacies (
    ...
    cursor.execute("""CREATE TABLE ventes (
    ...
    cursor.execute("""CREATE TABLE approvisionnement (
    ...
    cursor.executemany(
        ...
        [
            ("COOPHARMA Rabat",     1, 50000),
            ("PHARMA 5 Casablanca", 2, 80000),
            ("MAGHREB PHARMA",      2, 40000),
        ]
    )
    print(" 3 grossistes insérés")

    # VILLES
    for nom, (id_region, *_) in VILLES_CONFIG.items():
        cursor.execute("INSERT INTO villes (nom_ville, id_region) VALUES (?,?)",
                       (nom, id_region))
    print("5 villes insérées")

    # MÉDICAMENTS
    meds_list = gen_medicaments()
    cursor.executemany("INSERT INTO medicaments (nom, categorie, pfht) VALUES (?,?,?)", meds_list)
    cursor.execute("SELECT id_medicament, nom, categorie, pfht FROM medicaments")
    meds_inseres = cursor.fetchall()
    print(f" {len(meds_inseres)} médicaments insérés (T1:988, T2:9, T3:2, T4:1)")
    ...
    # VENTES
    cursor.execute("""SELECT id_patient, id_ville, id_pharmacie,
                      consommation_annuelle_boites FROM patients""")
    ...
    conn.commit()
    print("\n Base de données RÉELLE créée avec succès !")
    print(f"   Fichier     : pharmaciee_reel.db")
    print(f"   Régions     : 2")
    print(f"   Villes      : 5")
    print(f"   Grossistes  : 3")
    print(f"   Médicaments : {len(meds_inseres)} (T1:988 | T2:9 | T3:2 | T4:1)")
